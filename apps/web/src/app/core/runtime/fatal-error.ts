...
function formatError(error: unknown): string {
  ...
  return typeof error === 'string' ? error : 'Erro inesperado no carregamento.';
}
...
export function renderFatalError(error?: unknown): void {
  ...
  const host = document.querySelector('og-root') ?? document.body;
  ...
  panel.innerHTML = `
    <section style="width:min(100%,520px);padding:32px;border:1px solid #dbe3ef;border-radius:20px;background:#fff;box-shadow:0 18px 50px rgba(16,32,68,.12);text-align:center">
      ...
      <p style="margin:0 0 16px;line-height:1.55;color:#586987">Atualize a aplicação para carregar a versão mais recente.</p>
      <pre style="white-space:pre-wrap;overflow:auto;max-height:140px;margin:0 0 24px;padding:12px;border-radius:12px;background:#f4f7fb;color:#475569;font:12px/1.45 Consolas,monospace;text-align:left">${details}</pre>
